...
class TrajectoryDataset(Dataset):
    def __init__(self, path='Data/3D/new/csv/', size=None, model_type='A', flag='train'):
    ...
    def __read_data__(self):
        ...
        Y = [Class]
        '''
        NSamples = 500 if self.model_type == 'A' else 200
        
        d_type = {'train':0, 'valid':1, 'test':2, 'pred':3}
        lefts = [0, int(NSamples*0.7), int(NSamples*0.8), 0]
        rights = [int(NSamples*0.7), int(NSamples*0.8), NSamples, NSamples]
        ...
        for i, tr_type in enumerate(['Normal', 'Burntime', 'Xcpposition', "ThrustTiltAngle", 'Finbias']):
            ...
            for tr_i in range(left, right):
                ...
                if self.model_type == 'A':
                    ...
                    if i == 0: 
                        data_x_raw = [tr for _ in range(rights[d_type[self.flag]] - lefts[d_type[self.flag]])]
                        data_y_raw = [i for _ in range(rights[d_type[self.flag]] - lefts[d_type[self.flag]])]
                    else:
