...
class mlpEncoder(nn.Module):
    def __init__(self, input_channel, hidden, output_channel):
        ...
        self.model = nn.Sequential(
            nn.Linear(input_channel, hidden*(2**0), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**0), hidden*(2**1), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**1), hidden*(2**2), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**2), hidden*(2**1), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**1), output_channel, bias=False),
        )
...
class mlpDecoder(nn.Module):
    def __init__(self, input_channel, hidden, output_channel):
        ...
        self.model = nn.Sequential(
            nn.Linear(output_channel, hidden*(2**1), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**1), hidden*(2**2), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**2), hidden*(2**1), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**1), hidden*(2**0), bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(hidden*(2**0), input_channel, bias=False),
        )
